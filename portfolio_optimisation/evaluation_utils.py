...
from optimisation_utils import define_parameterised_optimisation_model, objective_function
import torch
...
def predict_then_optimise_follow_policy(dataloader_evaluation, prediction_model_trained, hyperparameters):
    ...
    with torch.no_grad():
        ...
        thetas = prediction_model_trained(context['Z'])
        # theta_1 = prediction_model_trained(context['Z'][:,0,...])['theta']
        theta_1 = thetas['theta'][:,0,...]

        decisions_optimised_1 = opt_layer(*[theta_1,context['initial_state']])
        ...
        for t in range(1,context['Z'].shape[1]):

            # theta_t = prediction_model_trained(context['Z'][:,t,...])['theta']
            theta_t = thetas['theta'][:,t,...]

            previous_state = decisions_now['z_state'][:,t,...].unsqueeze(-1) # Get latest state
...
def predict_then_optimise_follow_policy_singlestage(dataloader_evaluation, prediction_model_trained, hyperparameters):
    ...
    with torch.no_grad():
        ...
        for t in tqdm(range(1,context['Z'].shape[1]// hyperparameters['f']+1), leave = False, desc= 'evaluation'):
            ...
            previous_state = decisions_now['z_state'][:,int(t*hyperparameters['f']),...].unsqueeze(-1) # Get latest state
